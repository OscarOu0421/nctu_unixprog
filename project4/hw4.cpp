...
void chkbp(){
    ...
    if(ptrace(PTRACE_GETREGS, pid, 0, &regs) != 0)    error("ptrace(PTRACE_GETREGS)", errno);
...
void chkstat(){
    int status;
    if(waitpid(pid, &status, 0)< 0) error("waitpid", errno);
    if(WIFEXITED(status)){
        ...
        if(WSTOPSIG(status)==SIGTRAP){
            ...
            for(int i=0; i<(int)bp_list.size(); i++){
                if(bp_list[i].addr == regs.rip-1){
				    ...
				    if(ptrace(PTRACE_POKETEXT, pid, bp_list[i].addr, bp_list[i].orig_code) != 0)    error("ptrace(PTRACE_POKETEXT)", errno);
                    regs.rip--;
                    if(ptrace(PTRACE_SETREGS, pid, 0, &regs) != 0) error("ptrace(PTRACE_SETREGS)", errno);
                }
            }
...
void del(int id){
    ...
    for(int i=0; i<(int)bp_list.size(); i++){
        if(bp_list[i].id==id){
            ...
            if(ptrace(PTRACE_POKETEXT, pid, bp_list[id].addr, bp_list[id].orig_code) != 0)    error("ptrace(PTRACE_POKETEXT)", errno);
...
void si(){
    ...
    chkbp();
    if(ptrace(PTRACE_SINGLESTEP, pid, 0, 0)<0) error("ptrace(PTRACE_SINGLESTEP)", errno);
    chkstat();
}
void list(){
    for(int i=0; i<(int)bp_list.size(); i++)
...
void bp(unsigned long bp_addr){
    ...
    if((code & 0xff)==0xcc) //already set bp
        return;
    if(ptrace(PTRACE_POKETEXT, pid, bp_addr, (code & 0xffffffffffffff00) | 0xcc)!=0)    error("ptrace(POKETEXT)", errno);
...
void disasm(unsigned long dis_addr, int size){
    ...
    if(cs_open(CS_ARCH_X86, CS_MODE_64, &handle)!=CS_ERR_OK)    error("cs_open", errno);
...
void cont(){
    ...
    chkbp();
    if(ptrace(PTRACE_CONT, pid, 0, 0)<0)    error("ptrace(PTRACE_CONT)", errno);
    chkstat();
}
void run(){
    if(state==RUNNING){
...
void start(){
    ...
    if((pid = fork())<0)  error("fork", errno);
    else if(pid==0){
        if(ptrace(PTRACE_TRACEME, 0, 0, 0)<0)  error("ptrace(PTRACE_TRACEME)", errno);
        char **arg = {NULL};
        if(execvp(file_path.c_str(), arg)<0)    error("execvp", errno);
    }else{
        int status;
        if(waitpid(pid, &status, 0)< 0) error("waitpid", errno);
        if(ptrace(PTRACE_SETOPTIONS, pid, 0, PTRACE_O_EXITKILL)<0)    error("ptrace(PTRACE_SETOPTIONS)", errno);
...
int main(const int argc, const char *argv[]){
    ...
    while(1){
        ...
        arg1 = "";
        arg2 = "";
        if(cmd_table.size()>=1) cmd = cmd_table[0];
        if(cmd_table.size()>=2) arg1 = cmd_table[1];
        if(cmd_table.size()>=3) arg2 = cmd_table[2];
